...
import { LLaVAOneVisionProvider } from './providers/LLaVAOneVisionProvider';
...
export class ProviderFactory {
  ...
  static createProvider(config: LLMConfig, providerName?: string): LLMProvider {
    ...
    if (
      MODEL_PREFIXES.LLAVA_ONEVISION.some((prefix) => model.startsWith(prefix))
    ) {
      return new LLaVAOneVisionProvider(config);
    }
