...
import { settings } from '@ui-tars/shared/store';
import { VLMProviderV2 } from '@ui-tars/shared/types';

// MCPServerName에 LLaVA 추가
declare module '@agent-infra/shared' {
  interface MCPServerName {
    LLaVA: 'llava';
  }
}
...
export const actionRoute = t.router({
  ...
  saveBrowserSnapshot: t.procedure.input<void>().handle(async () => {
    ...
    try {
      ...
      // LLaVA-OneVision 분석 추가
      let visionAnalysis = null;
      if (settings.llavaProvider !== VLMProviderV2.none) {
        try {
          logger.info(
            '[actionRoute.saveBrowserSnapshot] Calling LLaVA-OneVision...',
          );
          const llavaResult = await mcpClient.callTool({
            client: MCPServerName.LLaVA,
                ...
                },
                {
                  ...
                  content: [
                    ...
                    },
                    {
                      type: 'image_url',
                      image_url: {
                        url: `data:image/${ext};base64,${screenshotMeta.data}`,
                        detail: 'high',
                      },
                    },
                  ],
          ...
          if (llavaResult.choices?.[0]?.message?.content) {
            visionAnalysis = llavaResult.choices[0].message.content;
            logger.info(
              '[actionRoute.saveBrowserSnapshot] Vision Analysis:',
              visionAnalysis,
            );
          }
        } catch (error) {
          logger.error(
            '[actionRoute.saveBrowserSnapshot] LLaVA-OneVision error:',
            error,
          );
        }
      }
      ...
      logger.error('[actionRoute.saveBrowserSnapshot] error:', error);
      throw error;
    }
  }),
