...
You should use the same language as the user input by default.
  ...
  public async run(): Promise<AwareResult> {
    ...
    // Thought가 생성되면 Vision Analysis 수행
    if (result.thought) {
      try {
        const { visionAnalysis } = await api.saveBrowserSnapshot();
        if (visionAnalysis) {
          // Vision Analysis를 Thought에 추가
          result.thought.thought = `${result.thought.thought}\n\n[Vision Analysis]\n${visionAnalysis}`;
        }
      } catch (error) {
  ...
  private async think(): Promise<AwareResult> {
