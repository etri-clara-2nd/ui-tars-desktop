...
export class EventManager {
  ...
  public async updateScreenshot(
    ...
    // 가장 최근의 브라우저 액션 이벤트 찾기
    const latestBrowserActionEvent = [...this.events]
      .reverse()
      .find(
        (event) =>
          event.type === EventType.ToolUsed &&
    ...
    if (!latestBrowserActionEvent) {
      return;
    }

    // 이벤트의 결과를 새로운 배열로 초기화
    const result: Array<{ type: string; path?: string; content?: string }> = [];
    ...
    // 이벤트 업데이트
    latestBrowserActionEvent.content = {
      ...latestBrowserActionEvent.content,
      result,
    };
